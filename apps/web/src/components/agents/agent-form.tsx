...
export const agentFormSchema = z.object({
  ...
  voiceSpeed: z.coerce.number().min(0.5).max(2.0).optional(),
  voiceEmotion: z.enum(['neutral', 'happy', 'calm', 'serious', 'friendly', 'professional']).optional(),
...
export function agentToFormValues(agent: Agent): AgentFormValues {
  return {
    ...
    wakeWordEnabled: (agent.config as Record<string,unknown>).wakeWordEnabled === true,
    avatarUrl: agent.config.avatarUrl ?? '',
      gender: agent.config.gender ?? 'neutral',
      voiceSpeed: agent.config.voiceSpeed ?? 1.0,
      voiceEmotion: agent.config.voiceEmotion ?? 'neutral',
      ...
      <div className="lg:col-span-2">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
              <div className="space-y-0.5">
                <Label>Wake Word Mode</Label>
                <FormDescription>
                  Agent starts silent. Say Zarax to wake up, Stop for now to pause.
                </FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
