...
export class BillingController {
  ...
  @Get("invoices")
  async invoices(@CurrentPrincipal() principal: Principal): Promise<Record<string, unknown>> {
    return {
      invoices: [],
      message: "Connect Stripe to view invoices. Set STRIPE_SECRET_KEY in environment variables.",
    };
  }
  ...
  @Get("payment-methods")
  async paymentMethods(@CurrentPrincipal() principal: Principal): Promise<Record<string, unknown>> {
    return {
      methods: [],
      message: "Connect Stripe to manage payment methods. Set STRIPE_SECRET_KEY in environment variables.",
    };
  }

  @RequirePermission(PERMISSIONS.TENANT_MANAGE_BILLING)
  @ApiOperation({ summary: "Buy credits (Stripe checkout)." })
  @Post("buy-credits")
  async buyCredits(
    ...
    if (!stripeKey) {
      return { error: "Stripe not configured. Set STRIPE_SECRET_KEY to enable payments.", checkoutUrl: null };
    }
    return { message: "Stripe checkout session would be created here.", amount: body.amount, checkoutUrl: null };
  }

  @RequirePermission(PERMISSIONS.TENANT_MANAGE_BILLING)
  @ApiOperation({ summary: "Upgrade plan (Stripe checkout)." })
  @Post("upgrade")
  async upgrade(
    @CurrentPrincipal() principal: Principal,
    ...
    const stripeKey = process.env.STRIPE_SECRET_KEY;
    if (!stripeKey) {
      return { error: "Stripe not configured. Set STRIPE_SECRET_KEY to enable upgrades.", checkoutUrl: null };
    }
