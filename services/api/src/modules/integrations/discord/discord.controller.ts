...
export class DiscordController {
  ...
  async webhook(@Body() body: DiscordInteraction): Promise<Record<string, unknown>> {
    ...
    if (body.type === 2 && body.data?.name === "zarax") {
      ...
      try {
        ...
        if (!tenant || !agent) {
          return { type: 4, data: { content: "Zarax is not configured yet." } };
        }
  ...
  @ApiOperation({ summary: "Check Discord bot status." })
  @Get("status")
  async status(): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const data = await res.json() as { id?: string; username?: string; error?: unknown };
