 ...
 * they're emitted as fixed-size AudioFrame objects at the correct sample rate.
 * The publisher is a thin wrapper — it does not buffer more than one frame in advance;
 * the TTS client drives timing by how fast it emits 'audio' events.
 */
export class LiveKitAudioPublisher {
  ...
  async start(): Promise<void> {
    ...
    await this.room.localParticipant.publishTrack(this.track, new TrackPublishOptions());
    this.published = true;
  }
  ...
  async push(pcmBuffer: Buffer): Promise<void> {
    ...
    const bytesPerFrame = samplesPerChannel * this.numChannels * 2; // 16-bit = 2 bytes

    for (let offset = 0; offset + bytesPerFrame <= pcmBuffer.length; offset += bytesPerFrame) {
      const slice = pcmBuffer.slice(offset, offset + bytesPerFrame);
